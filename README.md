...
| Rain Radar | A live precipitation radar map (via [RainViewer](https://www.rainviewer.com), no key needed) themed to match the dashboard, clamped to RainViewer's native zoom levels — the basemap tiles need a free [CARTO](https://carto.com/basemaps/apikey) API key |
